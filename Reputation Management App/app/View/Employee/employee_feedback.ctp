          ...
          <div class="col-sm-12> <!-- main col -->
			<div id="content-wrapper">
				<?php App::import('Controller', 'Dashboard');
				  ...
				  <?php // echo $this->element('topbar_business')?> <!-- !!! REMOVED DASH COLOR BOXES -->
										  ...
										  if(count($reviews) > 0)
												...
												{ 
												 ...
												 <tr>
																	...
																	<?php if($ratingstar == 5) { ?><td class="rate_star"><img src="<?php echo HTTP_ROOT?>app/webroot/img/5stars.png"></td><?php } ?>
																	...
																	<?php if($ratingstar == 1) { ?><td><img src="<?php echo HTTP_ROOT?>app/webroot/img/1star.png"></td><?php } ?>
																	 <td class="or-actions">
																		...
																		<?php if($value['BusinessReview']['approved'] == 1) {?>
																			  ...
																			  <span title="Active" style="cursor:pointer" class='statuschange' id="change<?php echo $value['BusinessReview']['id']?>" rel="<?php echo HTTP_ROOT.'Employee/approveReview/'.base64_encode($value['BusinessReview']['id']).'/'.$target[$value['BusinessReview']['approved']]?>">
																			  <img id="imge<?php echo $value['BusinessReview']['id']?>" src="<?php echo HTTP_ROOT?>img/action-suppress-1.png"></span>
																		   
																			<?php }else{ ?>
																		  
																			  <span title="Inactive" style="cursor:pointer" class='statuschange' id="change<?php echo $value['BusinessReview']['id']?>" rel="<?php echo HTTP_ROOT.'Employee/approveReview/'.base64_encode($value['BusinessReview']['id']).'/'.$target[$value['BusinessReview']['approved']]?>">
												 ...
												 <?php if($cnt > 4){?>
												 <a class="see_mr" style="margin-right: 41px; float:right; cursor:pointer; font-size:12px;" href="<?php echo HTTP_ROOT ?>Employee/feedBackSeeMore">SEE MORE REVIEWS</a>
												 <?php } ?>
										  ...
										  <tbody>
												...
												if(isset($Onlinereview))
												{
													foreach ($Onlinereview as $key => $value) {
																	...
																	$rating_image = round($value[0]['AverageRating']);
																	?>
																	<tr> 
																	<td class="rb-icon">
										...
										<?php if($value['Onlinereview']['ratingstar'] == 1) { ?><td class="or-stars"><img src="<?php echo HTTP_ROOT?>app/webroot/img/1star.png"></td><?php } ?>
											...
											<?php $target = array(0=>1,1=>0); ?>
													<?php if($value['Onlinereview']['confirmation'] == 1) {?>
														  ...
														  <span title="Active" style="cursor:pointer" class='statuschange1' id="change1<?php echo $value['Onlinereview']['id']?>" rel="<?php echo HTTP_ROOT.'Employee/onlinereviewsstatus/'.base64_encode($value['Onlinereview']['id']).'/'.$target[$value['Onlinereview']['confirmation']]?>">
														  <img id="imge1<?php echo $value['Onlinereview']['id']?>" src="<?php echo HTTP_ROOT?>img/action-suppress-1.png"></span>
													   
														<?php }else{ ?>
													  
														  <span title="Inactive" style="cursor:pointer" class='statuschange1' id="change1<?php echo $value['Onlinereview']['id']?>" rel="<?php echo HTTP_ROOT.'Employee/onlinereviewsstatus/'.base64_encode($value['Onlinereview']['id']).'/'.$target[$value['Onlinereview']['confirmation']]?>">
    ...
    $('.statuschange').on('click', function(){
 	 ...
 	 $('.statuschange1').on('click', function(){
       ...
       $.ajax({                   
                ...
                success: function (response) {
                  if(response[0]=='success'){
                     var newurl=ajax_url+'Employee/onlinereviewsstatus/'+response[1]+'/'+response[2];
                       if(response[2]==0){
                          var newdots=ajax_url+'img/action-suppress-1.png';
                          var title='Active';
                       }else{
